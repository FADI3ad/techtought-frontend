  ...
  <div class="min-h-screen bg-gray-50 flex flex-col">
    ...
    <div class="bg-primary-900 text-white py-12">
      <div class="max-w-[1340px] mx-auto px-6 lg:px-10">
        ...
        <p class="text-primary-200">Continue where you left off</p>
      </div>
    </div>
    ...
    <main class="flex-grow max-w-[1340px] mx-auto px-6 lg:px-10 py-12 w-full">
      ...
      <div v-else-if="courses.length === 0" class="bg-white border border-gray-200 p-12 rounded-2xl text-center max-w-2xl mx-auto shadow-sm">
        ...
        <button @click="router.push('/dashboard')" class="px-8 py-3 btn-global transition-all shadow-lg shadow-primary-100">Browse Courses</button>
      </div>
        ...
        <div v-for="course in courses" :key="course.id" class="group">
          ...
          <div class="mt-4">
            <button 
              ...
              class="w-full py-2 bg-primary-50 text-primary-700 rounded-lg font-bold text-sm hover:bg-primary-100 transition-colors group-hover:bg-primary-600 group-hover:text-white"
            >
              Continue Learning
            </button>
          </div>
