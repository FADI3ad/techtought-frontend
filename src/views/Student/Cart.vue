  ...
  <div class="min-h-screen bg-gray-50">
    <main class="max-w-[1340px] mx-auto px-6 py-12 lg:py-20">
        ...
        <div class="flex-grow space-y-4">
          <div v-for="item in cartStore.items" :key="item.id" class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col sm:flex-row gap-6 items-center">
            ...
            <div class="flex-grow">
              ...
              <p class="text-sm text-primary-600 font-medium mb-2">{{ item.course.category }}</p>
              <div class="flex items-center gap-4 text-sm text-gray-500">
                <span>By {{ item.course.instructor?.name || 'TechTought Instructor' }}</span>
              </div>
            </div>
          ...
          <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-xl sticky top-24">
            ...
            <div class="space-y-4 mb-8">
              ...
              <div class="border-t border-gray-100 pt-4 flex justify-between">
                ...
                <span class="text-2xl font-black text-primary-600">E£{{ cartStore.totalPrice }}</span>
              </div>
            </div>
            <button class="w-full btn-global transition-all shadow-lg shadow-primary-200 active:scale-[0.98]">
              Checkout Now
            </button>
      ...
      <div v-else class="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
        ...
        <router-link to="/" class="inline-flex px-8 py-3 btn-global transition-colors">
          Browse Courses
        </router-link>
      </div>
    </main>
